...
} from "https://www.gstatic.com/firebasejs/11.0.1/firebase-firestore.js";import { initializeApp } from "https://www.gstatic.com/firebasejs/11.0.1/firebase-app.js";
import {
    getFirestore, collection, addDoc, getDocs, doc,
    deleteDoc, updateDoc, query, orderBy, getDoc, setDoc
        ...
        snap.forEach(d => {
            ...
            <tr class="hover:bg-gray-50/60 transition-colors facturas-row">
                ...
                <td class="px-6 py-4 text-xs text-gray-600">
                    ...
                    <p>${f.razonSocial}</p>
                    <p>CP: ${f.cp} | Reg: ${f.regimenFiscal} | Uso: ${f.usoCfdi}</p>
                </td>
                <td class="px-6 py-4">
                ...
                <td class="px-6 py-4">
                    ${f.status === 'Pendiente' ? 
                        `<button onclick="completarFactura('${f.id}')" class="bg-black hover:bg-gray-800 text-white text-[10px] font-black uppercase px-3 py-1.5 rounded-lg transition">Marcar Lista</button>`
...
window.completarFactura = async (id) => {
    const url = prompt('Ingrese el link de la factura (PDF) (Opcional, puede dejarlo en blanco):');
    if(url !== null) {
        try {
            const dataToUpdate = { status: 'Completada', updatedAt: new Date().toISOString() };
            if (url.trim() !== '') {
                dataToUpdate.facturaUrl = url.trim();
            }
            await updateDoc(doc(db, 'facturas', id), dataToUpdate);
            showToast('Factura marcada como completada', 'success');
            loadFacturas();
